...
class TestMultiLinesAppend(unittest.TestCase):
    ...
    def test_char_by_char(self):
        ...
        self.assertEqual(lb.cpos_x_buffer, 0)
        self.assertEqual(lb.cpos_x_content, 0)
    # @NOTE: the difference between the first and second test
    # append-line is not very useful as it puts the cursor in the "wrong" place
    def test_line_by_line(self):
        ...
        for line in lines[0:5]:
            lb.append_line(line)
        v2 = lb.get_view()
        self.assertEqual(len(lb.content), 5)
