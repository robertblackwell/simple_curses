...
def min_size_for_views(views):
    """
    Compute the smallest rectangular area that will be large enough
    to hold all of the view objects provided.
    That means 
    -   find the max value of the set of {v.get_height() for v in views}
    -   find the max value of the set {v.get_width() for v in views}
    """
    h = 0
    w = 0
    for v in views:
...
class AppBase:
    """
    This is a base class which must be extended to create an application to use simple_curses.
    ##Application screen layout
    simple_curses applications all have a common format for the application screen.
    The screen is divided vertically into three areas:
    ### Top Menu
    Top area is for an application menu which is provided by a class TopmenuView.
    This class provides a left justified horizontal arrangement of a number of menu items
    ### View area or middle area
    A middle area is where most of the applcation activity takes place. 
    Depending on what is happening this area is filled by differents sets of widgets. 
    
    Each set of widgets that can fil this area are called a view and is represented by a class
    which has the word view in its name. The current list 
    -   DataEntryView, BannerView, HelpView, QuitView

    Views are selected by the top menu and there is a 1-1 correspondence bwteen top menu items and
    views that can be displayed by an application.
    ### Bottom or message are
    The final and bottom region of the app screen is devoted to messages. These are generated by the applcation
    and can be for any purpose.

    ## The register() function
    To define an application one must specify the list of top menu items and their corresponding views.
    This is done inside the `register()` function of a class derived from `AppBase`
    What does a derived class have to do:
    -   implement the method register()
    -   such an implementation sould set the properties:
        -   self.views: List[Union[DataEntryView, BlockTextView, HelpView, QuitView]] 
            -    This is the list of the views that can be displayed in the middle part of an application screen 
        -   self.topmenu_view: TopmenuView. 
            -   This single view should contain each of the top menu items (TopmenuItem) required
                for the app.
    """
    ...
    def register_views(self):
        ...
        The overide should set the properties:
        -   self.views
        -   self.topmenu_view

        self.views should be assigned to a variable of type List[views]
        """
